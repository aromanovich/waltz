...
|---|---|---|---|
| the log | `wal.Log` | `wal/memwal`, in process memory | `wal/waltest` — this repository's conformance suite: eighteen cases, one call |
| the database | `cold.Applier`, `cold.Watermarker` | `cold/memcold`, Temporal's own SQL persistence over in-process SQLite | Temporal's four exported persistence suites, which `memcold` runs unmodified |
