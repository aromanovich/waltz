...
**The boundary that produces most of this chapter is one sentence.** waltz implements no
persistence, so no suite here can run against storage, and every claim below the seams is therefore
somebody else's to make. That is a property of the library rather than a gap in its testing: a
library that shipped a store would have a store's evidence and a store's obligations, and this one
...
cannot be taken here, because taking it needs a cold store. That is why chapter 01's cost section is
...
## The shipped window has never been run against a real store
...
constancy is a property of **the applier**, which is a deployment's own code, and there is nothing
here to hold it. [Chapter 11](11-verification.md#the-guards) names it as one of the two guards a
deployment rebuilds rather than inherits, and this entry is the same fact stated as a limit: **no
run in this repository has ever sent a 256-mutation drain to anything.**
...
* **no fsync, no network, no quorum has ever been in the path** of anything in this repository. Every
  timing property of every suite here is the timing of a map;
...
## The cold store is a double, and it interprets nothing

`verify/coldtest` records what a drain carried and what watermark it moved. It does not interpret a
folded batch, deliberately: what a folded batch *means* has no specification apart from the
incumbent's behaviour, so a memory store that answered that question would be a rival to the thing
under test rather than a judge of it.

The consequence is the sharpest limit in this chapter. **Nothing here says a folded batch leaves a
cold store where the sequential path would have.** The acceptance says the fold is self-consistent at
volume; the unit tests say each rule does what it says. The claim that the *rules were conceived
correctly* needs a differential oracle against a real store, which is
[chapter 13](13-designs-that-were-rejected.md#a-unit-test-per-fold-rule)'s argument and a
deployment's to build.
...
```mermaid
graph LR
  A["go test ./..."] --> A1["the fold holds at volume, over an in-memory everything"]
  A --> A2["memwal satisfies the contract"]
  A --> A3["no decision the guards watch has been reverted"]
  A1 --> B["a claim about this library, in memory, on one workload"]
  A2 --> B
  A3 --> B
```

That is a claim about **this library, in memory, on one generated workload**, and about nothing
wider. In particular a green run is not, and cannot be:

* a claim about any storage;
* a claim that a composition over this library works end to end — the strongest available form of
  that is upstream's own functional suites against a real store, which
...
| the shipped window against a real store | 1 — a measurement |
...
| the cold store is a double | 2 — a differential oracle against a real store |
...
[Chapter 11](11-verification.md#three-levels-of-evidence) is where each instrument above is described
