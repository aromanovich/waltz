...
own. It puts a durable per-shard write-ahead log in front of a Temporal history shard's persistence
store and acknowledges a write as soon as the log holds it. In the shipped windowed configuration it
then folds many logged mutations into one later transaction against the store. That much sounds like
...
running Temporal server — can be exercised without installing anything, and both die with the
process. [Chapter 15](15-the-limits-of-the-evidence.md) says what that costs the evidence.
...
* **Six chapters number their sections, and the rest do not.** 05 to 10 are the procedural ones —
  ...
  section is a step and "§4" is a useful handle that other chapters cite. The rest are arguments
  rather than procedures, so their headings name the claim instead of the step.
