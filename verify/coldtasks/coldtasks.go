...
// coldtasks_test.go states both paginations plainly for that reason: it is what
// an implementer compares their own history-task store against, and a base that
// pages another way makes the suites above agree about pages it never returns.
//
// # Why it is a package and not a test file
//
// Because two packages' tests need it and neither may have the other's helpers
// (#177): `fold` owns what a page holds and `cycle` owns who may
// answer one, so the model that stands in for the plugin is asked the same
// questions from both sides. A second copy would be two models of one store,
// which is the duplication the ticket that split them exists to remove — and the
// first divergence would be a rule tested against a store the other half does not
// have. It sits under verify/ for the reason verify/mutgen does: it is an
// instrument, and the layer may not import it outside a test.
package coldtasks
