...
// for: the oracle applies each one to a cold store, the RAM probe keeps them
// alive, the acceptance only counts them. That is why the drain is a callback
// and not something this package performs.
//
...
// Acc is the accumulator being folded into, for a caller that reads it — the
// window still standing at the end of a run is part of the tail.
func (d *Driver) Acc() *fold.Accumulator { return d.acc }
