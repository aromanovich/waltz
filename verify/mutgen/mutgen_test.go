...
// knob that made it), it contains the shapes a recording cannot supply — chains,
...
//
// None of them needs a cluster. Whether the stream is one the store *accepts*
// is a different claim and needs one: TestGeneratedStreamIsAcceptedByTheStore in
// the repository root.
...
// clock reading — would pass a struct comparison and fail an oracle.
...
// TestAFullStreamIsMissingNothing is the coverage claim four cluster suites
// used to enumerate a field at a time, judged here where it costs nothing.
...
// TestRatioOneIsRecognisable is §P's acceptance criterion in test form. A corpus
// generated at WorkflowReuse 0 never touches a workflow twice, so its collapse
// ratio is exactly 1.00 — and 1.00 is the number a *broken fold* would also
// report. What makes the difference visible is that the ratio never travels
// without its knob, and that Collapses() answers no.
func TestRatioOneIsRecognisable(t *testing.T) {
	...
	require.Contains(t, report.String(), "WorkflowReuse 0.00",
		"the ratio may not be reported without the knob it was measured at (#12)")
...
// TestKeyReuseSetsThePerKeyCollapse: the second dial. At KeyReuse 0 every upsert
// names a key of its own — which is what upstream's generator does, and why a
// 200-long chain out of it collapsed by nothing (ratio 1.00, #12).
...
// and it collapses — which is the thing no recorded corpus can show, since no two
// of its 17 delta-carrying requests touch the same workflow.
//
...
// draining and letting the refused mutation head a fresh window. The oracle
// (#26) needs this loop, so the corpus's own test is where it is written down.
func TestChainsFold(t *testing.T) {
	...
	require.NotZero(t, refusals,
		"the default stream no longer exercises fold's refusal path, which #26 has to implement")
...
// reset-only shape is generated — the multi-part ones come from the recording,
// which has all of them — and in mode UpdateCurrent, which is what holds when the
// run being reset is the current one.
...
// TestBufferedEventsComeInBatchesAndAreCleared: one slot per mutation is the rule
// fold has to keep (#13), so what the corpus owes it is a chain of mutations each
// carrying a batch — and a clear, which only makes sense once something is
// buffered.
...
// TestTasksCoverAllFourCategories: the recording carries no history task at all,
// because upstream's generator maps every category to an empty slice
// (`tests/util.go`). Tasks in all four is therefore this package's job.
...
// since the oracle may want a stream that isolates state from queues.
...
// TestDeletesNameKeysThatWereThere is the other half of what the recording
// cannot supply: every one of its deletes names an id that was never upserted,
// so nothing in it exercises fold's upsert-vs-delete resolution. Here every
// delete names a key an earlier mutation of the same run upserted — and no
// mutation ever holds one key in both sets, which is the shape the store
// resolves wrongly and fold would then be blamed for.
...
// TestTombstonesAndRecreation: §P asks for the tombstone cases explicitly —
// both delete requests, and a workflow re-created after deletion. The pair's
// order is the one Temporal's delete flow uses, current pointer first.
...
func TestStreamSurvivesTheCodec(t *testing.T) {
	...
	compare := []cmp.Option{
		...
		// refuses to walk. A range delete carries one (#142).
