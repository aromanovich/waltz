// Package mutgen generates valid, deterministic mutation streams for the
// differential oracle. A seed must reproduce the same encoded bytes; generation
// therefore must not depend on time, UUIDs, Go map iteration, or protobuf maps.
package mutgen
...
type Config struct {
	...
	// an earlier mutation). Deletes that name a live key are the other thing
	// the recording cannot supply: every delete it contains names an id that
	// was never there.
	...
	// workflow task does). Batches do not merge — one slot per mutation (#13) —
	// so a chain of them is the only thing that exercises the rule.
	...
	// stream already holds, rather than a workflow step). It is the seventh
	// request kind (#142): tasks that arrive without any mutable state beside
	// them, which is what the four queue processors and the admin paths do.
...
// value for four of the knobs — WorkflowReuse 0 is precisely the corpus §P's
...
// reads as a pass and means the corpus never re-touched a workflow (#12).
type Report struct {
	Seed      int64
	Mutations int
	...
	// bound on what fold can merge, and the number §P's acceptance asks for.
	//
	// The history-task requests (#142) are excluded from the numerator on
	// purpose. They name no workflow, so counting them would raise the ratio on
	// a stream that gave the accumulator nothing more to collapse — the same
	// failure mode WorkflowReuse 0 has, arriving from the other direction.
...
// CompletesTasks reports whether the stream exercises the deletion rule at all.
// It is [Report.Collapses]' twin and is asked for the same reason: the count of
// ranges is not evidence, the count of tasks they cover is.
func (r Report) CompletesTasks() bool { return r.TasksCovered > 0 }
...
// materialised stream of that size is gigabytes of payload (#12).
