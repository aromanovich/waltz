...
func (g *Generator) newRun() *runState {
	return &runState{
		...
		// scheduled-event key of 0 is a shape no real mutation carries. That is
		// the whole of the reason now: the incumbent store's read path used to
		// fail every read of such a run with "got multiple executions rows" —
		// its row classification took an item row with event_id 0 and no name
		// for the execution row itself, found by the oracle's attribution
		// readback (#26) and fixed in that store. Moving this to 0 would
		// therefore work, and it would still be wrong: the generator models
		// what upstream writes, and every recorded measurement is a function of
		// the streams these seeds produce.
		nextActivity: 1,
	}
}
...
func (g *Generator) emitCreate(w *workflowState, mode p.CreateWorkflowMode) error {
	...
	req := &p.InternalCreateWorkflowExecutionRequest{
		...
		// stamped by whoever drives the request — apply, or the oracle's
		// sequential path — and a copy here would be a second source of truth.
		Mode:                mode,
...
// shapes — reset + new, reset + current — come from the recording, which has all
// of them; what it has none of is one inside a chain, which is what this is for.
...
func (g *Generator) mutation(w *workflowState, r *runState) (p.InternalWorkflowMutation, error) {
	...
	// of them matters — they do not merge (#13), so two mutations must stay two
...
// categories a persisted queue has. The recording supplies none of these at all
// — upstream's own generator maps every category to an empty slice — so this is
// the only place the task path fold has to concatenate is exercised.
...
// every delete names a key the run actually holds — the thing a recorded corpus
// cannot supply, where every delete names an id that was never there (#12).
...
func (g *Generator) executionState(r *runState) *persistencespb.WorkflowExecutionState {
	...
	// the way out of a blob — the one difference the oracle's identity control
	// has to normalise (#14).
...
// there. A range over an interval nothing was ever written into is a mutation
// the oracle will judge and a rule nothing exercised — the same failure the
// collapse ratio has at WorkflowReuse 0, one ticket later.
...
//
// **The bump is a microsecond, and a nanosecond is not enough**: a stored fire
// time is microseconds, so a maximum a nanosecond above a task's fire time
// truncates to that same fire time and the store's DELETE covers nothing. The oracle
// caught it as a row the folded path dropped and the sequential path kept — a
// corpus that generated such a range would judge the deletion rule in name only,
// which is the same trap [Config.RangeCompleteRate] warns about, arriving
// through the store's resolution instead of through the key space. The stream's
// fire times are a second apart, so a microsecond separates any two of them.
