...
// order of the two things is what A9 rests on — reads the record back while the
// call is in flight.
type store struct {
	p.ExecutionStore
...
// The order is the whole of A9: a process killed inside the call must leave a
// call nothing accounts for, never an entry nothing accounts for. Nothing but
// this can check it — a chaos run kills nodes, and the record it leaves is the
