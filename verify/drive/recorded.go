...
// Recorder drives one shard's calls and writes down what happened to each. It
// is the unit every chaos assertion is stated over, which is why it is here
// rather than in the binary that used to hold two copies of it: two fsynced
// lines with a deadline between them, and the gap between the lines is the
// third outcome class.
//
// Three things about it are the checker's contract rather than convenience, and
// all three used to be a caller's to remember:
//
//   - **the call line is durable before the store is touched.** A9 — no entry
//     without a call — rests on it: a process killed inside the call must leave
//     a call with no outcome, not an entry nothing accounts for;
...
type Recorder struct {
	...
	// which for a chaos node means no bound at all, so the third outcome class
	// then only arrives by a kill.
