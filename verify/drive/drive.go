...
// reachable. In the research prototype one of this package's callers was a
// binary — a chaos node, which drives a stream under `kill -9` and has no test
// to fail — so everything here returns an error instead. A fixture is one
// adapter over it and such a node is the other, which is what keeps the driving
// half a single implementation both of them are judged through.
//
...
// checker's business and what it costs is the oracle's. The dependency on
// verify/mutgen only points this way — a generator that could reach the thing
// driving it would be tuned to it.
package drive
