...
import (
	...
	"github.com/aromanovich/waltz/verify/drive"
	"github.com/aromanovich/waltz/verify/foldrun"
	"github.com/aromanovich/waltz/verify/mutgen"
	"github.com/aromanovich/waltz/wal"
)

// envAcceptanceMutations overrides the stream length. The acceptance record's
// number is 10^6; the default here is a tenth of it, which keeps the test in
// test-all's budget while still being an order of magnitude past anything the
// cluster-bound runs can afford.
const envAcceptanceMutations = "WAL_ACCEPTANCE_MUTATIONS"

const defaultAcceptanceMutations = 100_000

// acceptanceWindow is how many mutations fold accumulates before a drain. It
// stands in for P3's drain policy, which does not exist yet: big enough for
...
// that is KindDelete alone, this suite's definition and not the oracle's.
...
// is a function of them and reads as a constant without them (#12, #14). A
...
func TestAcceptanceFoldNoCluster(t *testing.T) {
	...
	// is not a measurement (#12). A tenth of the volume is plenty: this run
