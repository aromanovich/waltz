...
// WithBuffered is one buffered-events blob, named by its own bytes so a written
// one can be told from every other in a batch.
func WithBuffered(name string) MutationOpt {
	return func(m *p.InternalWorkflowMutation) { m.NewBufferedEvents = named(name) }
}
...
// WithSnapshotTasks is [WithTaskRows] for the shapes that carry a snapshot.
func WithSnapshotTasks(category tasks.Category, list ...p.InternalHistoryTask) SnapshotOpt {
	return func(s *p.InternalWorkflowSnapshot) {
		s.Tasks = map[tasks.Category][]p.InternalHistoryTask{category: list}
	}
}
