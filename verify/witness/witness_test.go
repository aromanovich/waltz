...
// The witness's own tests (#174, continuing #154's rule): the module whose job
// is to catch a silent pass must be judgeable itself, and a witness that cannot
// go red is that failure with an extra step. So the negative cases are the
// deliverable and the green ones are the control: each healthy run below is
// spoiled in exactly one way, and the claim is that the witness says so — by
// name, so that a red run tells its reader which half of the layer went
...
// carried batches, and no condition ever reaching a drain (#138).
...
//
// The two spoil tables that used to stand here — one per window, seventeen
// cases matching a substring of the message — are the leave-one-out now.
// Fifteen of them staged a defect a named claim catches, and asserting that
// *some* error mentioned a phrase could not say which claim caught it, or
...
// TestReadingAroundTheLayerIsAsRedAsReadingThroughIt (#80): a suite that reads
...
// layer in it would answer the attribution question (#130) with the layer's
// own behaviour, which is the one thing it exists to exclude.
...
// TestTheHistoryTaskPathIsPinnedWhereItIsExercised (#142): both task calls go
// into the log in the runs that make them and in no other run — the count is
// zero without a layer and non-zero wherever the run writes tasks, and a run
// reporting the old shape (#33) is a task path that went round the accumulator.
...
//
// This is stronger than "the witness went red", which is what the two spoil
// tables below assert and what they asserted before the claims had names: a
// spoil that trips three claims satisfied a substring match, so nothing said
// whether the claim under test was one of the three.
//
