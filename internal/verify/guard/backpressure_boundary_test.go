...
// be made inside either package, because it is about both.
//
...
//     and their tasks stay out of the DLQ;
...
// passthrough mode the base store is the plugin; in intercept mode it is the
// cycle, and the obligation does not change.
...
// will actually travel in production: the binary's own composition, a real
...
//
// It is a real store rather than a plain `nil`, because the condition
// authority hands the layer the base store's own two reads. A method value
// taken on a nil interface panics *where it is taken*, so a nil here crashes
// the test on its first write, before the refusal it is about, and takes the
// whole test binary — every case scheduled after this file included — with it.
//
// A windowed cycle is what this test builds, since backpressure needs a tail,
// and an intercepted write there legitimately reads the pre-window current row
// through this store: that is fold.Delegated.Settle closing the residual, and
...
// that read through (baserow.Store): the response type
...
// the one this fixture is really guarding. The rest of the interface stays
// promoted from the embedded nil: nothing here can reach it without taking a
// method value first, which is the case this file already covers.
